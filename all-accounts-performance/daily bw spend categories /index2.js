...
(function() {
    ...
    function findDatasetByQueryName(queryName) {
        console.log(`Spend Categories: Searching ALL datasets by queryName (index-independent search)`);
        
        for (let key in datasets) {
            const dataset = datasets[key];
            if (dataset.queryName === queryName) {
                console.log(`Spend Categories: ✓ FOUND "${queryName}" at index ${key} (would work at ANY index)`);
                return dataset;
            }
        }
        
        console.log(`Spend Categories: ✗ No dataset found with queryName: "${queryName}"`);
        return null;
    }
    ...
    function loadDatasetContent() {
        ...
        console.log('Spend Categories: Looking for dataset with queryName:', CONFIG.datasetName);
        ...
        //     console.log('Spend Categories: ✓ Using fallback INDEX:', CONFIG.fallbackIndex);
        //     targetDataset = datasets[CONFIG.fallbackIndex];
        // }

        if (targetDataset) {
            rawData = targetDataset.content || targetDataset;
            console.log('Spend Categories: Data loaded, rows:', rawData.length);
            console.log('Spend Categories: ✓ SUCCESS - Working purely by queryName, no index fallback needed!');
        } else {
            console.error('Spend Categories: Dataset not found - queryName search failed and fallback is disabled');
            rawData = [];
        }
    }

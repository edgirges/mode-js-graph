// =============================================================================
// New Campaign Groups Starting each Month for that Month
// =============================================================================

(function() {
    'use strict';

    console.log('=== New Campaign Groups Starting each Month for that Month Chart script loaded! ===');
    ...
    function init() {
        const canvas = document.getElementById('campaignGroupsMonthChart');
        const toggles = document.querySelector('.campaign-groups-month-toggles');
    ...
    function attemptInit() {
        //console.log('New Campaign Groups Starting each Month for that Month: Attempting initialization...');
        if (init()) {
            //console.log('New Campaign Groups Starting each Month for that Month: Successfully initialized!');
            return;
        }
        //console.log('New Campaign Groups Starting each Month for that Month: Init failed, retrying...');
    ...
    function loadData() {
        try {
            ...
            if (extractedMetrics.length > 0) {
                ...
                console.warn('New Campaign Groups Starting each Month for that Month: No metrics extracted from dataset');
            }
        } catch (error) {
            console.error('New Campaign Groups Starting each Month for that Month: Error loading data:', error);
        }
    }

    function findDatasetByQueryName(queryName) {
        console.log(`Campaign Groups Month: Searching ALL datasets by queryName (index-independent search)`);
        
        for (let key in datasets) {
            const dataset = datasets[key];
            if (dataset.queryName === queryName) {
                console.log(`Campaign Groups Month: ✓ FOUND "${queryName}" at index ${key} (would work at ANY index)`);
                return dataset;
            }
        }
        
        console.log(`Campaign Groups Month: ✗ No dataset found with queryName: "${queryName}"`);
        return null;
    }
    ...
    function loadDatasetContent() {
        ...
        console.log('Campaign Groups Month: Looking for dataset with queryName:', CONFIG.datasetName);
        ...
        //     console.log('Campaign Groups Month: ✓ Using fallback INDEX:', CONFIG.fallbackIndex);
        //     targetDataset = datasets[CONFIG.fallbackIndex];
        // }

        if (targetDataset) {
            rawData = targetDataset.content || targetDataset || [];
            console.log('Campaign Groups Month: Data loaded, rows:', rawData.length);
            console.log('Campaign Groups Month: ✓ SUCCESS - Working purely by queryName, no index fallback needed!');
        } else {
            console.error('Campaign Groups Month: Dataset not found - queryName search failed and fallback is disabled');
            rawData = [];
        }
    }
    ...
    function processData() {
        if (!rawData.length || !dynamicMetrics.length) {
            //console.log('New Campaign Groups Starting each Month for that Month: No data to process. rawData length:', rawData.length, 'dynamicMetrics length:', dynamicMetrics.length);
            return;
        }
...
function createMetricToggles() {
    const container = document.querySelector('.campaign-groups-month-toggles');
    ...
    const metricsHeader = document.querySelector('.campaign-groups-month-metrics-title');
    if (metricsHeader && !document.getElementById('campaign-groups-month-select-all-btn')) {
        const selectAllBtn = document.createElement('button');
        selectAllBtn.id = 'campaign-groups-month-select-all-btn';
        selectAllBtn.className = 'select-all-btn';
        selectAllBtn.textContent = 'Select All';
        selectAllBtn.onclick = selectAllMetrics;
        
        const deselectAllBtn = document.createElement('button');
        deselectAllBtn.id = 'campaign-groups-month-deselect-all-btn';
...
function switchTimeRange(timeRange) {
    ...
    document.querySelectorAll('.campaign-groups-month-controls .control-btn')
        .forEach(btn => {
...
function resetZoom() {
    ...
    window.CampaignGroupsMonthChart = {
        loadData,
        switchTimeRange,
        toggleZoom,
        resetZoom,
